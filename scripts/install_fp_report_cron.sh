...
LINE="30 8 * * 1 cd \"$ROOT\" && bash scripts/fp_report.sh >>\"$LOG\" 2>&1; cp -f fp-report.json .cache/dashboard-live/fp-report.json 2>/dev/null || true $MARK"
...
echo "  Pazartesi 08:30 — fp_report.sh -> $LOG (morning_gate 08:00 sonrasi)"
