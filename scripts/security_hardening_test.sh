...
grep -q 'bypassLoginRateLimit' dashboard/src/lib/loginRateLimit.ts || fail "login rate limit prod bypass guard yok"
test -x scripts/hardening_rollback_gate.sh || fail "hardening_rollback_gate.sh yok"
grep -qE '\.bak\.' scripts/apply_internet_facing_hardening.sh || fail "apply_internet_facing yedek yok"
