...
| **False positive** | **%0.5** (200 benign) | Yüksek (regex yok) | Orta | CRS'ye bağlı |
| **Ban gecikmesi** | **~17 ms** (ipset) | Saniye–dakika | Saniye | Log-only / ayrı entegrasyon |
...
| **Bizim fark** | Aynı nginx log'unda CRS + anomaly + **17 ms** ipset ban |
...
**TR:**  
Linux Log Guardian — nginx log → OWASP CRS → kernel ban, tek paket, MIT. Ölçülmüş: %100 gerçek saldırı recall (1K+10K), %0.5 FP, ~17 ms ban, 5 dk soak PASS. ModSec'ten hızlı değiliz; entegre hat + kanıt PDF ile farklıyız.

**EN:**  
Linux Log Guardian — nginx log → OWASP CRS → kernel ban in one MIT stack. Measured: 100% real-attack recall (1K+10K), 0.5% FP, ~17 ms ban, 5m soak PASS. Not faster than ModSec inline; differentiated by integrated pipeline + public proof.
