...
const I18N = {
  "tr": {
    ...
    "install.intro": "<p>Ubuntu/Debian üzerinde <strong>sıfırdan Core kurulumu</strong> (~15 dk). İki yol vardır: hazır <strong>.deb paketi</strong> (önerilen, derleme yok) veya kaynak koddan derleme. Laptop / VirtualBox VM'de XDP olmadan da çalışır. Tam dokümantasyon: <code>docs/QUICKSTART_NGINX.md</code> · <code>docs/LAPTOP_OPS.md</code>.</p>",
    "install.expect_label": "Beklenen çıktı (örnek):",
    "install.deb_title": "A) Sıfır sunucu — .deb paketi (önerilen)",
    "install.deb_body": "<p>Derleme gerektirmez. Paket binary, systemd unit'leri, kurallar, script'ler ve <code>rules.conf.template</code> içerir. <strong>Upgrade güvenli:</strong> mevcut <code>/etc/log-guardian/rules.conf</code> (API token, ayarlar) paket tarafından <em>silinmez</em>.</p><p>Paketi indirin (GitHub Releases) veya geliştirici makinede <code>bash scripts/build_deb.sh</code> ile üretin → <code>dist/log-guardian_*.deb</code>.</p>",
    ...
    "install.src_body": "<p>Repoyu klonlayıp yerelde derlersiniz. Geliştirme ve özelleştirme için uygundur.</p>",
    ...
    "download.intro": "<p><strong>Sıfır sunucu kurulumu</strong> için yukarıdaki <a href=\"#kurulum\">Kurulum rehberi → A) .deb</a> adımlarını izleyin. Paket kaynakları:</p><ul><li><strong>GitHub Releases</strong> — hazır <code>log-guardian_*_amd64.deb</code> (yayınlandığında)</li><li><strong>Kaynak repodan üretim</strong> — geliştirici makinede aşağıdaki komutlar</li></ul><p>Kurulum yapmadan paket bütünlüğü: <code>bash scripts/test_deb_local.sh</code></p>",
  ...
  "en": {
    ...
    "install.intro": "<p><strong>Fresh Core install</strong> on Ubuntu/Debian (~15 min). Two paths: ready-made <strong>.deb package</strong> (recommended, no compile) or build from source. Works without XDP on laptop / VirtualBox VM. Full docs: <code>docs/QUICKSTART_NGINX.md</code> · <code>docs/LAPTOP_OPS.md</code>.</p>",
    "install.expect_label": "Expected output (sample):",
    "install.deb_title": "A) Fresh server — .deb package (recommended)",
    "install.deb_body": "<p>No compile step. The package ships binaries, systemd units, rules, scripts and <code>rules.conf.template</code>. <strong>Upgrade-safe:</strong> an existing <code>/etc/log-guardian/rules.conf</code> (API token, settings) is <em>not</em> overwritten.</p><p>Download from GitHub Releases or build on a dev machine with <code>bash scripts/build_deb.sh</code> → <code>dist/log-guardian_*.deb</code>.</p>",
    ...
    "install.src_body": "<p>Clone the repo and build locally. Best for development and customization.</p>",
    ...
    "download.intro": "<p>For a <strong>fresh server install</strong>, follow <a href=\"#kurulum\">Setup guide → A) .deb</a> above. Package sources:</p><ul><li><strong>GitHub Releases</strong> — prebuilt <code>log-guardian_*_amd64.deb</code> (when published)</li><li><strong>Build from source repo</strong> — commands below on a dev machine</li></ul><p>Integrity check without installing: <code>bash scripts/test_deb_local.sh</code></p>",
    "download.block": "bash scripts/build_deb.sh\n# → dist/log-guardian_*.deb\nbash scripts/test_deb_local.sh",
