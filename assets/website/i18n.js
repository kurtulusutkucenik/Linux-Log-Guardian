...
const I18N = {
  "tr": {
    ...
    "hero.badge_os": "🇹🇷 Türk yapımı",
    "hero.badge_mit": "Açık kaynak · MIT",
    ...
    "install.intro": "<p>Bu rehber Ubuntu/Debian üzerinde <strong>sıfırdan üretim kurulumu</strong> içindir. Laptop veya VirtualBox VM'de XDP olmadan da çalışır. Tam dokümantasyon: repo içi <code>docs/QUICKSTART_NGINX.md</code> ve <code>docs/LAPTOP_OPS.md</code>.</p>",
    "install.req_title": "Gereksinimler",
    ...
    "install.block1": "git clone <repo> log-guardian && cd log-guardian\nmake -j$(nproc)\nsudo bash install.sh",
    ...
    "install.s4_title": "Doğrulama",
    "install.s4_body": "<p>Sağlık, alarm özeti ve Prometheus metriklerini kontrol edin. Laptop kapısı için <code>post_install_verify.sh</code> tek komutta özet verir.</p>",
    ...
    "dashboard.where": "<strong>Önemli:</strong> Dashboard bu tanıtım sitesinde <em>çalışmaz</em>. <code>localhost:8443</code>, komutları çalıştırdığınız <strong>kendi Linux makinenizde</strong> açılır — ziyaretçinin tarayıcısındaki localhost sizin sunucunuz değildir.",
    "dashboard.body": "<p>Next.js tabanlı <strong>Pro dashboard</strong>: filo durumu, ban geçmişi, test sonuçları ve kanıt PDF'leri. TLS ile kurulu sunucuda <code>:8443</code> portunda dinler. İnternete açmadan önce mutlaka <code>laptop_harden.sh</code> ile sertleştirin.</p>",
    ...
    "dashboard.access_row2": "Kendi Ubuntu/Debian sunucusunda <code>install.sh</code> + <code>dashboard_stack.sh</code> sonrası, <strong>aynı makinede</strong> tarayıcıdan <code>https://localhost:8443</code>.",
    "dashboard.access_row3_label": "Uzak VPS",
    "dashboard.access_row3": "SSH tüneli veya (firewall açıksa) <code>https://SUNUCU_IP:8443</code>. Demo parola kapatılmadan internete açmayın.",
    ...
    "warn.title": "⚠️ İnternete açmadan önce",
    ...
    "download.intro": "<p>Kaynak derlemeden paket üretmek için <code>build_deb.sh</code>. Kurulum yapmadan bütünlük kontrolü: <code>test_deb_local.sh</code>.</p>",
    "download.block": "bash scripts/build_deb.sh\n# → dist/log-guardian_*.deb\nbash scripts/test_deb_local.sh",
    "download.verify": "Paket doğrulama (kurulum yapmadan): <code>bash scripts/test_deb_local.sh</code>",
    "evidence.title": "Kanıt paketi",
    "evidence.gate": "Laptop kapısı: <code>bash scripts/laptop_sprint_gate.sh</code>",
    "evidence.sync": "Güncelle: <code>bash scripts/sync_evidence_pack.sh</code>",
    "laptop.title": "Laptop doğrulama matrisi",
    "laptop.intro": "<p>Geliştirme makinesinde hızlı kapı: sprint gate, 1 saatlik soak ve 3 dk demo. VM'de 72h soak için <code>laptop_soak_72h.sh --start</code>.</p>",
    ...
    "contact.body": "<p>Sorular, iş birliği veya katkı için yazın. Hata bildirimi ve pull request'ler açık kaynak ruhuyla memnuniyetle karşılanır.</p>",
    "contact.email_label": "E-posta:",
    "footer": "Linux Log Guardian · Türk yapımı · MIT · açık kaynak · <a href=\"mailto:kurtulusutkucenikcontact@gmail.com\">kurtulusutkucenikcontact@gmail.com</a>"
  },
  "en": {
    ...
    "about.body": "<p><strong>Linux Log Guardian</strong> is a <strong>open-source (MIT)</strong> self-hosted security stack built in Turkey. Instead of juggling Fail2ban, ModSecurity and separate scripts, you get one chain: <em>read nginx logs → match OWASP CRS rules → ban at kernel level</em>.</p><p>An independent community project — full code review, customization and on-prem hosting with no cloud lock-in.</p><ul><li>Transparent source; community contributions welcome</li><li>Binary: <code>log-guardian</code> · package: <code>log-guardian_*.deb</code></li><li><strong>Core</strong> alone is production-ready (~15 min)</li><li>Dashboard, Grafana, fleet = optional <strong>Pro</strong> layer</li></ul>",
    ...
    "install.intro": "<p>Step-by-step <strong>production setup</strong> on Ubuntu/Debian. Works without XDP on laptop or VirtualBox VM. Full docs: <code>docs/QUICKSTART_NGINX.md</code> and <code>docs/LAPTOP_OPS.md</code>.</p>",
    "install.req_title": "Requirements",
    ...
    "install.block_nginx": "# examples/nginx-log-guardian.conf → site config\nSTRICT=1 bash scripts/check_nginx_log_format.sh\nsudo nginx -t && sudo systemctl reload nginx",
    "install.s4_title": "Verification",
    "install.s4_body": "<p>Check health, alarms and Prometheus metrics. <code>post_install_verify.sh</code> summarizes on laptop.</p>",
    "install.block_verify": "sudo log-guardian --health\nsudo log-guardian --status\nbash scripts/post_install_verify.sh\ncurl -s http://127.0.0.1:9091/metrics | head",
    ...
    "dashboard.where": "<strong>Important:</strong> The dashboard does <em>not</em> run on this marketing site. <code>localhost:8443</code> is on <strong>your own Linux machine</strong> where you ran install — a visitor's browser localhost is not your server.",
    "dashboard.body": "<p>The <strong>Pro dashboard</strong> (Next.js): fleet status, ban history, tests and evidence PDFs. Listens on <code>:8443</code> with TLS on the host you installed. Harden with <code>laptop_harden.sh</code> before exposing to the internet.</p>",
    ...
    "dashboard.access_row2": "After <code>install.sh</code> + <code>dashboard_stack.sh</code> on their Ubuntu/Debian host, opens <code>https://localhost:8443</code> <strong>on that same machine</strong>.",
    "dashboard.access_row3_label": "Remote VPS",
    "dashboard.access_row3": "SSH tunnel or (if firewall allows) <code>https://SERVER_IP:8443</code>. Do not expose with demo password.",
    "dashboard.req_title": "Extra requirements for dashboard",
    "dashboard.req_body": "<ul><li><strong>Core</strong> only needs nginx + sudo — dashboard <em>not required</em></li><li><strong>Dashboard</strong> needs Docker + <code>dashboard_stack.sh</code></li><li><strong>Grafana</strong> additionally <code>grafana_stack.sh</code> (optional)</li></ul>",
    ...
    "warn.title": "⚠️ Before exposing to the internet",
    ...
    "download.intro": "<p>Build package with <code>build_deb.sh</code>. Integrity check without install: <code>test_deb_local.sh</code>.</p>",
    "download.block": "bash scripts/build_deb.sh\n# → dist/log-guardian_*.deb\nbash scripts/test_deb_local.sh",
    "download.verify": "Package validation (no install): <code>bash scripts/test_deb_local.sh</code>",
    ...
    "laptop.intro": "<p>Quick dev gate: sprint gate, 1h soak and 3 min demo. For 72h VM soak: <code>laptop_soak_72h.sh --start</code>.</p>",
    "laptop.block": "SKIP_PHASE100=1 bash scripts/laptop_sprint_gate.sh\nSOAK_1H=1 bash scripts/laptop_soak_72h.sh --start\nSKIP_WEBHOOK=1 bash scripts/demo_3min.sh",
    "contact.title": "Contact & contributions",
    "contact.body": "<p>Questions, collaboration or contributions — get in touch. Bug reports and pull requests welcome.</p>",
    "contact.email_label": "Email:",
    "footer": "Linux Log Guardian · Made in Turkey · MIT · open source · <a href=\"mailto:kurtulusutkucenikcontact@gmail.com\">kurtulusutkucenikcontact@gmail.com</a>"
  },
  "de": {
    ...
    "dashboard.where": "<strong>Wichtig:</strong> Das Dashboard läuft <em>nicht</em> auf dieser Marketing-Seite. <code>localhost:8443</code> ist auf <strong>Ihrem Linux-Rechner</strong>, auf dem Sie installiert haben — localhost im Browser eines Besuchers ist nicht Ihr Server.",
    "dashboard.body": "<p><strong>Pro-Dashboard</strong> (Next.js): Fleet-Status, Ban-Historie, Tests und Evidence-PDFs. Hört auf <code>:8443</code> mit TLS auf dem installierten Host. Vor Internet-Exposition mit <code>laptop_harden.sh</code> härten.</p>",
    "dashboard.access_title": "Wer greift wann zu?",
    "dashboard.access_row1_label": "Seitenbesucher",
    "dashboard.access_row1": "Liest diese Seite, lädt Quellcode oder .deb — öffnet <strong>nicht</strong> Port 8443.",
    "dashboard.access_row2_label": "Installierender",
    "dashboard.access_row2": "Nach <code>install.sh</code> + <code>dashboard_stack.sh</code> auf Ubuntu/Debian: <code>https://localhost:8443</code> <strong>auf derselben Maschine</strong>.",
    "dashboard.access_row3_label": "Remote-VPS",
    "dashboard.access_row3": "SSH-Tunnel oder (wenn Firewall erlaubt) <code>https://SERVER_IP:8443</code>. Nicht mit Demo-Passwort exponieren.",
    "dashboard.req_title": "Zusätzliche Dashboard-Anforderungen",
    "dashboard.req_body": "<ul><li><strong>Core</strong> braucht nur nginx + sudo — Dashboard <em>nicht erforderlich</em></li><li><strong>Dashboard</strong> braucht Docker + <code>dashboard_stack.sh</code></li><li><strong>Grafana</strong> zusätzlich <code>grafana_stack.sh</code> (optional)</li></ul>",
  ...
  "fr": {
    ...
    "dashboard.where": "<strong>Important :</strong> le dashboard ne tourne <em>pas</em> sur ce site. <code>localhost:8443</code> est sur <strong>votre machine Linux</strong> où vous avez installé — le localhost du visiteur n'est pas votre serveur.",
    "dashboard.body": "<p><strong>Dashboard Pro</strong> (Next.js) : fleet, historique des bans, tests et PDF de preuves. Écoute sur <code>:8443</code> en TLS. Durcir avec <code>laptop_harden.sh</code> avant exposition internet.</p>",
    "dashboard.access_title": "Qui y accède, et quand ?",
    "dashboard.access_row1_label": "Visiteur du site",
    "dashboard.access_row1": "Lit cette page, télécharge source ou .deb — n'ouvre <strong>pas</strong> le port 8443.",
    "dashboard.access_row2_label": "Installateur",
    "dashboard.access_row2": "Après <code>install.sh</code> + <code>dashboard_stack.sh</code> sur Ubuntu/Debian : <code>https://localhost:8443</code> <strong>sur la même machine</strong>.",
    "dashboard.access_row3_label": "VPS distant",
    "dashboard.access_row3": "Tunnel SSH ou (si firewall) <code>https://SERVER_IP:8443</code>. Ne pas exposer avec le mot de passe démo.",
    "dashboard.req_title": "Prérequis supplémentaires dashboard",
    "dashboard.req_body": "<ul><li><strong>Core</strong> : nginx + sudo — dashboard <em>non requis</em></li><li><strong>Dashboard</strong> : Docker + <code>dashboard_stack.sh</code></li><li><strong>Grafana</strong> : aussi <code>grafana_stack.sh</code> (optionnel)</li></ul>",
  ...
  "es": {
    ...
    "dashboard.where": "<strong>Importante:</strong> el dashboard <em>no</em> corre en este sitio. <code>localhost:8443</code> está en <strong>tu máquina Linux</strong> donde instalaste — el localhost del visitante no es tu servidor.",
    "dashboard.body": "<p><strong>Dashboard Pro</strong> (Next.js): estado de fleet, historial de bans, tests y PDFs de prueba. Escucha en <code>:8443</code> con TLS. Endurecer con <code>laptop_harden.sh</code> antes de exponer a internet.</p>",
    "dashboard.access_title": "¿Quién accede y cuándo?",
    "dashboard.access_row1_label": "Visitante del sitio",
    "dashboard.access_row1": "Lee esta página, descarga fuente o .deb — <strong>no</strong> abre el puerto 8443.",
    "dashboard.access_row2_label": "Instalador",
    "dashboard.access_row2": "Tras <code>install.sh</code> + <code>dashboard_stack.sh</code> en Ubuntu/Debian: <code>https://localhost:8443</code> <strong>en la misma máquina</strong>.",
    "dashboard.access_row3_label": "VPS remoto",
    "dashboard.access_row3": "Túnel SSH o (si firewall) <code>https://SERVER_IP:8443</code>. No exponer con contraseña demo.",
    "dashboard.req_title": "Requisitos extra del dashboard",
    "dashboard.req_body": "<ul><li><strong>Core</strong> solo necesita nginx + sudo — dashboard <em>no requerido</em></li><li><strong>Dashboard</strong>: Docker + <code>dashboard_stack.sh</code></li><li><strong>Grafana</strong>: además <code>grafana_stack.sh</code> (opcional)</li></ul>",
  ...
  "pt": {
    ...
    "dashboard.where": "<strong>Importante:</strong> o dashboard <em>não</em> roda neste site. <code>localhost:8443</code> está na <strong>sua máquina Linux</strong> onde instalou — o localhost do visitante não é seu servidor.",
    "dashboard.body": "<p><strong>Dashboard Pro</strong> (Next.js): fleet, histórico de bans, testes e PDFs. Escuta em <code>:8443</code> com TLS. Endurecer com <code>laptop_harden.sh</code> antes de expor à internet.</p>",
    "dashboard.access_title": "Quem acessa e quando?",
    "dashboard.access_row1_label": "Visitante do site",
    "dashboard.access_row1": "Lê esta página, baixa fonte ou .deb — <strong>não</strong> abre a porta 8443.",
    "dashboard.access_row2_label": "Instalador",
    "dashboard.access_row2": "Após <code>install.sh</code> + <code>dashboard_stack.sh</code> no Ubuntu/Debian: <code>https://localhost:8443</code> <strong>na mesma máquina</strong>.",
    "dashboard.access_row3_label": "VPS remoto",
    "dashboard.access_row3": "Túnel SSH ou (se firewall) <code>https://SERVER_IP:8443</code>. Não exponha com senha demo.",
    "dashboard.req_title": "Requisitos extras do dashboard",
    "dashboard.req_body": "<ul><li><strong>Core</strong> só precisa nginx + sudo — dashboard <em>não obrigatório</em></li><li><strong>Dashboard</strong>: Docker + <code>dashboard_stack.sh</code></li><li><strong>Grafana</strong>: também <code>grafana_stack.sh</code> (opcional)</li></ul>",
  ...
  "ru": {
    ...
    "dashboard.where": "<strong>Важно:</strong> dashboard <em>не</em> работает на этом сайте. <code>localhost:8443</code> — на <strong>вашей Linux-машине</strong>, где вы установили; localhost посетителя — не ваш сервер.",
    "dashboard.body": "<p><strong>Pro dashboard</strong> (Next.js): fleet, история ban, тесты и PDF. Слушает <code>:8443</code> с TLS. Перед internet — <code>laptop_harden.sh</code>.</p>",
    "dashboard.access_title": "Кто и когда получает доступ?",
    "dashboard.access_row1_label": "Посетитель сайта",
    "dashboard.access_row1": "Читает страницу, скачивает исходники или .deb — <strong>не</strong> открывает порт 8443.",
    "dashboard.access_row2_label": "Установивший",
    "dashboard.access_row2": "После <code>install.sh</code> + <code>dashboard_stack.sh</code> на Ubuntu/Debian: <code>https://localhost:8443</code> <strong>на той же машине</strong>.",
    "dashboard.access_row3_label": "Удалённый VPS",
    "dashboard.access_row3": "SSH-туннель или (если firewall) <code>https://SERVER_IP:8443</code>. Не открывайте с demo-паролем.",
    "dashboard.req_title": "Доп. требования для dashboard",
    "dashboard.req_body": "<ul><li><strong>Core</strong> — nginx + sudo; dashboard <em>не обязателен</em></li><li><strong>Dashboard</strong> — Docker + <code>dashboard_stack.sh</code></li><li><strong>Grafana</strong> — также <code>grafana_stack.sh</code> (опционально)</li></ul>",
  ...
  "ar": {
    ...
    "dashboard.where": "<strong>مهم:</strong> لوحة التحكم <em>لا</em> تعمل على هذا الموقع. <code>localhost:8443</code> على <strong>جهاز Linux</strong> حيث ثبّت — localhost للزائر ليس خادمك.",
    "dashboard.body": "<p><strong>Pro dashboard</strong> (Next.js): fleet، سجل الحظر، الاختبارات وPDF. يستمع على <code>:8443</code> بـ TLS. قوِّ بـ <code>laptop_harden.sh</code> قبل الإنترنت.</p>",
    "dashboard.access_title": "من يصل ومتى؟",
    "dashboard.access_row1_label": "زائر الموقع",
    "dashboard.access_row1": "يقرأ هذه الصفحة وينزّل المصدر أو .deb — <strong>لا</strong> يفتح المنفذ 8443.",
    "dashboard.access_row2_label": "المثبت",
    "dashboard.access_row2": "بعد <code>install.sh</code> + <code>dashboard_stack.sh</code> على Ubuntu/Debian: <code>https://localhost:8443</code> <strong>على نفس الجهاز</strong>.",
    "dashboard.access_row3_label": "VPS بعيد",
    "dashboard.access_row3": "نفق SSH أو (إذا سمح firewall) <code>https://SERVER_IP:8443</code>. لا تفتح بكلمة demo.",
    "dashboard.req_title": "متطلبات إضافية للوحة",
    "dashboard.req_body": "<ul><li><strong>Core</strong> يحتاج nginx + sudo — dashboard <em>غير مطلوب</em></li><li><strong>Dashboard</strong>: Docker + <code>dashboard_stack.sh</code></li><li><strong>Grafana</strong>: أيضاً <code>grafana_stack.sh</code> (اختياري)</li></ul>",
  ...
  "zh": {
    ...
    "dashboard.where": "<strong>重要：</strong> dashboard <em>不</em>在此营销站运行。<code>localhost:8443</code> 在<strong>您安装的那台 Linux 机器</strong>上 — 访客的 localhost 不是您的服务器。",
    "dashboard.body": "<p><strong>Pro dashboard</strong> (Next.js)：fleet 状态、封禁历史、测试与证据 PDF。在已安装主机 <code>:8443</code> TLS 监听。上网前请用 <code>laptop_harden.sh</code> 加固。</p>",
    "dashboard.access_title": "谁何时访问？",
    "dashboard.access_row1_label": "网站访客",
    "dashboard.access_row1": "阅读本页、下载源码或 .deb — <strong>不会</strong>打开 8443 端口。",
    "dashboard.access_row2_label": "安装者",
    "dashboard.access_row2": "在 Ubuntu/Debian 上 <code>install.sh</code> + <code>dashboard_stack.sh</code> 后，<strong>同一机器</strong>打开 <code>https://localhost:8443</code>。",
    "dashboard.access_row3_label": "远程 VPS",
    "dashboard.access_row3": "SSH 隧道或（防火墙允许时）<code>https://SERVER_IP:8443</code>。勿用 demo 密码暴露公网。",
    "dashboard.req_title": "Dashboard 额外要求",
    "dashboard.req_body": "<ul><li><strong>Core</strong> 只需 nginx + sudo — dashboard <em>非必需</em></li><li><strong>Dashboard</strong> 需 Docker + <code>dashboard_stack.sh</code></li><li><strong>Grafana</strong> 另需 <code>grafana_stack.sh</code>（可选）</li></ul>",
