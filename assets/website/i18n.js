...
function scheduleHeavyMotion() {
  ...
  try {
    if (sessionStorage.getItem("lg_enter_rev7") === "1") {
      run();
      return;
    }
  } catch (_) {}
...
function forceFinishEnter() {
  ...
  try {
    sessionStorage.setItem("lg_enter_rev7", "1");
  } catch (_) {}
  ensureSpineMotion();
...
function initEnterCurtain() {
  ...
  let seen = false;
  try {
    seen = sessionStorage.getItem("lg_enter_rev7") === "1";
  } catch (_) {}
...
function initCoverflowNav() {
  const wrap = document.querySelector(".lg-motion-rail-wrap.lg-scroll-stack");
  const rail = document.getElementById("lg-motion-rail");
  if (!wrap || !rail || initCoverflowNav.done) return;
  initCoverflowNav.done = true;
  ...
  function metrics() {
    const rect = wrap.getBoundingClientRect();
    const vh = window.innerHeight;
    const total = Math.max(wrap.offsetHeight - vh, 0);
    const topAbs = window.scrollY + rect.top;
    return { rect, vh, total, topAbs };
  }

  function frontIndex() {
    const m = metrics();
    if (m.total <= 0) return 0;
    const scrolled = Math.min(Math.max(-m.rect.top, 0), m.total);
  ...
  function goTo(i) {
    ...
    const p = n > 1 ? i / (n - 1) : 0;
    window.scrollTo({
      top: Math.round(m.topAbs + p * m.total),
  ...
  const nav = document.getElementById("lg-motion-rail-nav");
  if (nav) {
    const prev = nav.querySelector("[data-rail-prev]");
    const next = nav.querySelector("[data-rail-next]");
    const auto = nav.querySelector("[data-rail-auto]");
    if (prev) {
  ...
  function bindDots() {
    const dw = document.getElementById("lg-motion-coverflow-dots");
    if (!dw) return;
    [...dw.querySelectorAll(".lg-motion-coverflow-dot")].forEach((d, i) => {
      if (d.dataset.lgNavBound === "1") return;
      d.dataset.lgNavBound = "1";
  ...
  bindDots();
  window.setTimeout(bindDots, 400);
  ...
  window.addEventListener(
    ...
    () => {
      ...
      snapT = window.setTimeout(() => {
        ...
        const m = metrics();
        const scrolled = Math.min(Math.max(-m.rect.top, 0), m.total);
        const raw = (scrolled / m.total) * (n - 1);
        const nearest = Math.round(raw);
        if (nearest <= 0 || nearest >= n - 1) return;
...
function initHorizontalRail() {
  ...
  if (document.body.classList.contains("lg-clean-v2")) {
    ...
    if (desktop && wrap) {
      ...
      const scrollVh = Math.max(520, n * 100);
      wrap.style.setProperty("--lg-rail-scroll-h", scrollVh + "vh");
      wrap.style.height = scrollVh + "vh";
      initCoverflowRail();
      requestAnimationFrame(() => requestAnimationFrame(() => initCoverflowNav()));
      return;
    }
    initShowcaseGrid();
    return;
...
function initCompareScroll() {
  ...
  revealRows.forEach((row) => {
    row.classList.add("lg-compare-row");
    row.classList.remove("lg-compare-row-on");
    row.style.removeProperty("--lg-compare-i");
  });
  if (prefersReducedMotion()) {
    revealRows.forEach((row) => row.classList.add("lg-compare-row-on"));
  }

  function compareInJourney() {
    const rect = wrap.getBoundingClientRect();
    const navH = parseFloat(getComputedStyle(document.documentElement).getPropertyValue("--nav-h")) || 72;
    const vh = window.innerHeight;
    return rect.top <= navH + 20 && rect.bottom >= vh * 0.58;
  }

  function revealByProgress(p) {
    if (!revealRows.length || prefersReducedMotion()) return;
    if (!compareInJourney()) return;
    const n = revealRows.length;
    revealRows.forEach((row, i) => {
      const threshold = Math.max(0, (i / n) * 0.92 - 0.01);
      if (p >= threshold) {
        row.classList.add("lg-compare-row-on");
        row.style.setProperty("--lg-compare-i", String(i));
      }
    });
  }

  function animateCompareChrome(p) {
    const outer = wrap.querySelector(".lg-motion-compare-outer");
    const cols = wrap.querySelector(".lg-compare-cols");
    const table = rail.querySelector(".lg-compare-table");
    const inJ = compareInJourney();
    const lift = inJ ? (1 - Math.min(1, p * 2.8)) * 14 : 14;
    const fade = inJ ? Math.min(1, 0.2 + p * 1.15) : 0.2;
    if (outer) {
      outer.style.opacity = String(fade);
      outer.style.transform = "translate3d(0, " + lift.toFixed(1) + "px, 0)";
    }
    if (cols) {
      cols.style.opacity = String(inJ ? Math.min(1, 0.25 + p * 0.9) : 0.25);
      cols.style.transform = "translate3d(" + (-18 + p * 18).toFixed(1) + "px, 0, 0)";
    }
    if (table && !prefersReducedMotion()) {
      const tilt = inJ ? (p - 0.5) * 2.5 : 0;
  ...
  function update() {
    ...
    revealByProgress(p);
    animateCompareChrome(p);
    const inJ = compareInJourney();
    wrap.classList.toggle("lg-compare-active", inJ && p > 0.02 && p < 0.98);
    const head = wrap.querySelector(".lg-motion-compare-head");
    if (head && inJ) head.classList.add("lg-visible");
...
function initSectionPager() {
  ...
  function syncActive() {
    if (Date.now() < pagerLockUntil) return;
    const mid = window.innerHeight * 0.42;
    let best = 0;
    let bestDist = Infinity;
    sections.forEach((sec, i) => {
...
function initMotion() {
  ...
  steps.forEach((fn) => {
    try {
      ...
      console.error("[lg-motion]", fn.name || "step", err);
    }
  });
}
